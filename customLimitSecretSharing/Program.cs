...
namespace customLimitSecretSharing
{
    class Program
    {
        static void Main(string[] args)
        {
            Console.Write("Введите число сотрудников N (целое и > 1): ");
            //Число сотрудников, оно должно быть больше единицы
            int n = 1;
            while (n == 1 || n < 1)
            {
                n = Convert.ToInt32(Console.ReadLine());
                if (n == 1 || n < 1) Console.WriteLine("N должно быть больше 1, повторите ввод: ");
            }

            //Пороговое число (минимальное число сотрудников, которые могут восстановить ключ)
            Console.Write("Введите пороговое число h: ");
            int h = Convert.ToInt32(Console.ReadLine());

            //Количество сегментов, на которые разбивается ключ вектор
            Program program = new Program();
            int segments = program.Factorial(n) / (program.Factorial(h - 1) * program.Factorial(n - h + 1));
            ...
            CustomLimit customLimit = new CustomLimit();
            customLimit.ShareMethod(segments, n, h, ref accessMap);
